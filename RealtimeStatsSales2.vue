...
<script>
  ...
  export default {
    ...
    methods: {
      ...mapActions('app', ['fetch', 'fetchCall', 'showError','setBusy']),
      async initData(isRequery) {
        this.setBusy({status: true})
        ...
        await this.fetchCall({
          onCall: async() => {
            ...
            this.detail = await this.fetch({
              api: 'report/salesConfirmationReport4j',
              params: {
            ...
            if(this.detail.status===0){
              setTimeout(()=>{
                this.initData(true)
                this.setBusy({status: false});
              },2000)
            }else {
              this.report=this.detail.data;
